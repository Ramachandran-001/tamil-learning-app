...
import { MOCK_ENGLISH_UNITS } from "../../data/mockEnglishData";
import { MOCK_MATHS_UNITS } from "../../data/mockMathsData";
import { MOCK_SCIENCE_UNITS } from "../../data/mockScienceData";
import { MOCK_SOCIAL_UNITS } from "../../data/mockSocialData";
...
export const GenericSubjectView = () => {
  const { subjectId } = useParams();
  const subject = SUBJECTS_DATA.find((item) => item.id === subjectId);
  
  const getSubjectData = () => {
    switch(subjectId) {
      case "english": return MOCK_ENGLISH_UNITS;
      case "maths": return MOCK_MATHS_UNITS;
      case "science": return MOCK_SCIENCE_UNITS;
      case "social": return MOCK_SOCIAL_UNITS;
      case "tamil":
      default:
        return MOCK_TAMIL_UNITS;
    }
  };

  const UNITS_DATA = getSubjectData();

  if (!UNITS_DATA) return <Navigate to="/learning/subjects" />;
  ...
  const [selectedUnitId, setSelectedUnitId] = useState(UNITS_DATA[0]?.id || "");
  const [activeTab, setActiveTab] = useState("iyals");

  const selectedUnit =
    UNITS_DATA.find((u) => u.id === selectedUnitId) ||
    UNITS_DATA[0];
  const isSelectedUnlocked = selectedUnit ? isUnitUnlocked(selectedUnit.id) : false;
  const selectedUnitPrerequisite = selectedUnit ? getUnitPrerequisite(selectedUnit.id) : null;

  const totalLessons = UNITS_DATA.reduce(
    (acc, u) => acc + u.lessons.length,
    0,
  );
  const lessonIds = new Set(UNITS_DATA.flatMap((unit) => unit.lessons.map((lesson) => lesson.id)));
  ...
  const manappadamLessons = UNITS_DATA.flatMap((u) =>
  ...
  return (
    <div className="space-y-6 text-left pb-12">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/20 border border-emerald-400/40 text-emerald-300 text-xs font-bold">
              ...
              <span>தமிழ்நாடு பாடத்திட்டம் · 10-ஆம் வகுப்பு</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-black font-sans tracking-tight text-white capitalize">
              {subject?.nameTamil || `10-ஆம் வகுப்பு ${subjectId}`} · கற்றல் பாதை
            </h1>
            <p className="text-xs sm:text-sm text-slate-200 leading-relaxed">
              ஒவ்வொரு பாடத்தையும் படித்து, பயிற்சி செய்து, முன்னேற்றத்தைப் பெறுங்கள். அடுத்த பகுதி முந்தைய பகுதியை முடித்ததும் திறக்கும்.
            </p>
          </div>
          ...
          <div className="shrink-0 p-5 bg-slate-950/70 backdrop-blur-xl rounded-2xl border border-emerald-500/30 min-w-[220px] text-center shadow-lg">
            <span className="text-xs text-emerald-300 font-semibold block mb-1 capitalize">
              இந்தப் பாட முன்னேற்றம்
            </span>
      ...
      <div className="flex items-center gap-2 border-b border-slate-800 pb-2 overflow-x-auto">
        ...
        >
          பாடப்பாதை · {UNITS_DATA.length} பகுதிகள்
        </button>
        {subjectId === "tamil" && <button
        ...
        <div className="space-y-6">
          ...
          {!isSelectedUnlocked && selectedUnitPrerequisite ? (
            ...
            <>
              ...
              <LessonPathMap unit={selectedUnit} subjectId={subjectId} />
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Link to={`/practice/${subjectId}/${selectedUnit.id}`}>
                      <Button
                        variant="outline"
                        size="sm"
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-4 pt-3 border-t border-slate-800/80 flex items-center justify-between">
                          ...
                          {isLessonUnlockedNow ? (
                            <Link to={`/lesson/${subjectId}/${lesson.id}`}>
                              <Button
